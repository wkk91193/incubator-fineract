...
public class LoanWorkbookPopulator extends AbstractWorkbookPopulator {
	...
	public void populate(Workbook workbook) {
		...
		groupSheetPopulator.populate(workbook);
		personnelSheetPopulator.populate(workbook);
		productSheetPopulator.populate(workbook);
		...
		setClientAndGroupDateLookupTable(loanSheet, clientSheetPopulator.getClients(), groupSheetPopulator.getGroups(),
				LOOKUP_CLIENT_NAME_COL, LOOKUP_ACTIVATION_DATE_COL,LINK_ACCOUNT_ID);
	}
	...
	private void setRules(Sheet worksheet) {
		...
		CellRangeAddressList productNameRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), PRODUCT_COL, PRODUCT_COL);
		CellRangeAddressList loanOfficerRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), LOAN_OFFICER_NAME_COL, LOAN_OFFICER_NAME_COL);
		CellRangeAddressList submittedDateRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), SUBMITTED_ON_DATE_COL, SUBMITTED_ON_DATE_COL);
		...
		CellRangeAddressList noOfRepaymentsRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), NO_OF_REPAYMENTS_COL, NO_OF_REPAYMENTS_COL);
		CellRangeAddressList repaidFrequencyRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), REPAID_EVERY_FREQUENCY_COL, REPAID_EVERY_FREQUENCY_COL);
		CellRangeAddressList loanTermRange = new CellRangeAddressList(1, SpreadsheetVersion.EXCEL97.getLastRowIndex(),
				LOAN_TERM_COL, LOAN_TERM_COL);
		CellRangeAddressList loanTermFrequencyRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), LOAN_TERM_FREQUENCY_COL, LOAN_TERM_FREQUENCY_COL);
		CellRangeAddressList interestFrequencyRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), NOMINAL_INTEREST_RATE_FREQUENCY_COL,
		...
		CellRangeAddressList amortizationRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), AMORTIZATION_COL, AMORTIZATION_COL);
		CellRangeAddressList interestMethodRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), INTEREST_METHOD_COL, INTEREST_METHOD_COL);
		CellRangeAddressList intrestCalculationPeriodRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), INTEREST_CALCULATION_PERIOD_COL,
				INTEREST_CALCULATION_PERIOD_COL);
		CellRangeAddressList repaymentStrategyRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), REPAYMENT_STRATEGY_COL, REPAYMENT_STRATEGY_COL);
		CellRangeAddressList arrearsToleranceRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), ARREARS_TOLERANCE_COL, ARREARS_TOLERANCE_COL);
		CellRangeAddressList graceOnPrincipalPaymentRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), GRACE_ON_PRINCIPAL_PAYMENT_COL,
				GRACE_ON_PRINCIPAL_PAYMENT_COL);
		CellRangeAddressList graceOnInterestPaymentRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), GRACE_ON_INTEREST_PAYMENT_COL,
				GRACE_ON_INTEREST_PAYMENT_COL);
		CellRangeAddressList graceOnInterestChargedRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), GRACE_ON_INTEREST_CHARGED_COL,
				GRACE_ON_INTEREST_CHARGED_COL);
		CellRangeAddressList approvedDateRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), APPROVED_DATE_COL, APPROVED_DATE_COL);
		CellRangeAddressList disbursedDateRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), DISBURSED_DATE_COL, DISBURSED_DATE_COL);
		CellRangeAddressList paymentTypeRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), DISBURSED_PAYMENT_TYPE_COL, DISBURSED_PAYMENT_TYPE_COL);
		...
		CellRangeAddressList lastrepaymentDateRange = new CellRangeAddressList(1,
				SpreadsheetVersion.EXCEL97.getLastRowIndex(), LAST_REPAYMENT_DATE_COL, LAST_REPAYMENT_DATE_COL);
		...
		DataValidationConstraint loanTypeConstraint = validationHelper
				.createExplicitListConstraint(new String[] { "Individual", "Group", "JLG" });
		...
		DataValidationConstraint approvalDateConstraint = validationHelper
				.createDateConstraint(DataValidationConstraint.OperatorType.BETWEEN, "=$F1", "=TODAY()", "dd/mm/yy");
		...
		DataValidationConstraint loanTermConstraint = validationHelper
				.createIntegerConstraint(DataValidationConstraint.OperatorType.GREATER_OR_EQUAL, "=$L1*$M1", null);
		...
		DataValidationConstraint lastRepaymentDateConstraint = validationHelper
				.createDateConstraint(DataValidationConstraint.OperatorType.BETWEEN, "=$H1", "=TODAY()", "dd/mm/yy");
	...
	private void setDefaults(Sheet worksheet) {
		...
		for (Integer rowNo = 1; rowNo < 1000; rowNo++) {
			...
			writeFormula(REPAID_EVERY_FREQUENCY_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"REPAYMENT_FREQUENCY_\",$D"
					+ (rowNo + 1) + "))),\"\",INDIRECT(CONCATENATE(\"REPAYMENT_FREQUENCY_\",$D" + (rowNo + 1) + ")))");
			...
			writeFormula(LOAN_TERM_COL, row, "IF(ISERROR($L" + (rowNo + 1) + "*$M" + (rowNo + 1) + "),\"\",$L"
					+ (rowNo + 1) + "*$M" + (rowNo + 1) + ")");
			writeFormula(LOAN_TERM_FREQUENCY_COL, row, "$N" + (rowNo + 1));
			writeFormula(NOMINAL_INTEREST_RATE_FREQUENCY_COL, row,
					"IF(ISERROR(INDIRECT(CONCATENATE(\"INTEREST_FREQUENCY_\",$D" + (rowNo + 1)
							+ "))),\"\",INDIRECT(CONCATENATE(\"INTEREST_FREQUENCY_\",$D" + (rowNo + 1) + ")))");
			writeFormula(NOMINAL_INTEREST_RATE_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"INTEREST_\",$D"
					+ (rowNo + 1) + "))),\"\",INDIRECT(CONCATENATE(\"INTEREST_\",$D" + (rowNo + 1) + ")))");
			...
			writeFormula(INTEREST_CALCULATION_PERIOD_COL, row,
					"IF(ISERROR(INDIRECT(CONCATENATE(\"INTEREST_CALCULATION_\",$D" + (rowNo + 1)
							+ "))),\"\",INDIRECT(CONCATENATE(\"INTEREST_CALCULATION_\",$D" + (rowNo + 1) + ")))");
			writeFormula(ARREARS_TOLERANCE_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"ARREARS_TOLERANCE_\",$D"
					+ (rowNo + 1) + "))),\"\",INDIRECT(CONCATENATE(\"ARREARS_TOLERANCE_\",$D" + (rowNo + 1) + ")))");
			...
			writeFormula(GRACE_ON_PRINCIPAL_PAYMENT_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"GRACE_PRINCIPAL_\",$D"
					+ (rowNo + 1) + "))),\"\",INDIRECT(CONCATENATE(\"GRACE_PRINCIPAL_\",$D" + (rowNo + 1) + ")))");
			writeFormula(GRACE_ON_INTEREST_PAYMENT_COL, row,
					"IF(ISERROR(INDIRECT(CONCATENATE(\"GRACE_INTEREST_PAYMENT_\",$D" + (rowNo + 1)
							+ "))),\"\",INDIRECT(CONCATENATE(\"GRACE_INTEREST_PAYMENT_\",$D" + (rowNo + 1) + ")))");
			writeFormula(GRACE_ON_INTEREST_CHARGED_COL, row,
					"IF(ISERROR(INDIRECT(CONCATENATE(\"GRACE_INTEREST_CHARGED_\",$D" + (rowNo + 1)
							+ "))),\"\",INDIRECT(CONCATENATE(\"GRACE_INTEREST_CHARGED_\",$D" + (rowNo + 1) + ")))");
